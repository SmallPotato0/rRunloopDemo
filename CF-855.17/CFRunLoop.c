...
// t==0 is a synonym for "main thread" that always works(当t==0时,代表着主线程)
CF_EXPORT CFRunLoopRef _CFRunLoopGet0(pthread_t t) {
    if (pthread_equal(t, kNilPthreadT)) {//如果是主线程
    //当前线程等于主线程
	t = pthread_main_thread_np();
    }
    //加锁
    __CFSpinLock(&loopsLock);
    if (!__CFRunLoops) {//如果当前runloops存在(主线程)
        //解锁
    ...
    __CFSpinUnlock(&loopsLock);
    if (!loop) {//如果不存在
	CFRunLoopRef newLoop = __CFRunLoopCreate(t);//重新创建
        __CFSpinLock(&loopsLock);//加锁
        //重新获取当前线程的runloop
	loop = (CFRunLoopRef)CFDictionaryGetValue(__CFRunLoops, pthreadPointer(t));
	if (!loop) {//存在
	    CFDictionarySetValue(__CFRunLoops, pthreadPointer(t), newLoop);//线程--runloop存进字典
	    loop = newLoop;
	}
        // don't release run loops inside the loopsLock, because CFRunLoopDeallocate may end up taking it
        __CFSpinUnlock(&loopsLock);//解锁
	CFRelease(newLoop);
    }
...
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    //如果CFRunLoopGetCurrent已经被释放,返回kCFRunLoopRunFinished
    if (__CFRunLoopIsDeallocating(rl)) return kCFRunLoopRunFinished;
    __CFRunLoopLock(rl);
    
    //根据modeName找到对应的model
    CFRunLoopModeRef currentMode = __CFRunLoopFindMode(rl, modeName, false);
    //如果currentMode为空或者currentMode没有source/timer/observer
    if (NULL == currentMode || __CFRunLoopModeIsEmpty(rl, currentMode, rl->_currentMode)) {
	Boolean did = false;
	if (currentMode) __CFRunLoopModeUnlock(currentMode);
	__CFRunLoopUnlock(rl);
        //kCFRunLoopRunHandledSource 进入loop时参数说处理完事件就返回,kCFRunLoopRunFinished source/timer/observer一个都没有了
    ...
    //kCFRunLoopEntry, 通知 Observers: RunLoop 即将进入 loop。
	if (currentMode->_observerMask & kCFRunLoopEntry ) __CFRunLoopDoObservers(rl, currentMode, kCFRunLoopEntry);
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
    
    //kCFRunLoopExit ,通知 Observers: RunLoop 即将退出
...
//用指定的mode启动,seconds:允许设置runloop的超时
